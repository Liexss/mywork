$(document).ready(function() {
  ...
  $("#btn").click(function( ){
    if($('#name').val()==""||$('#money').val()==""||$('#begintime').val()==""||$('#endtime').val()==""){
      ...
      if($flag) return;
      var da={'enclosure':urll,'content':$('#content').val(),'name':$('#name').val(),'money':$('#money').val(),'begintime':$('#begintime').val(),'endtime':$('#endtime').val(),'teacher_id':$("#selteacher option:selected").attr("id"),'reward_id':$txt};
      console.log(da);
      $.ajax({
        url: '../ajax_php/change_reward.php',
        type : 'POST',
        dataType : 'JSON',
        data : da, 
        success : function(msg) {
